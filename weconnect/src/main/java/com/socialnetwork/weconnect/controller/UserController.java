...
public class UserController {
	...
	public ApiResponse<List<FileInfo>> getListImageByUser() {
		List<FileInfo> fileInfos = storageService.loadAllByUserName().map(path -> {
			...
			String url = MvcUriComponentsBuilder.fromMethodName(PostController.class, "getImage", imageName).build()
					.toString();
	...
	public ApiResponse<UserInforRestponse> getInformationUser() {
		return ApiResponse.<UserInforRestponse>builder().result(userService.getInformationUser()).build();
	}
	...
	public ApiResponse<UserInforRestponse> editInformationUser(@RequestBody UserInforRequest userInforRequest) {
		return ApiResponse.<UserInforRestponse>builder().result(userService.editInformationUser(userInforRequest))
				.build();
	}
